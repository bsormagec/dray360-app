set @START_DATE=date_sub(CURRENT_TIMESTAMP, interval 120 hour);
set @END_DATE=null;
...
from (
    ...
    from (
        select
            ...
            ,(select json_unquote(json_extract(max(s2.status_metadata), '$.pdf_page_count')) from t_job_state_changes as s2 where s.request_id = s2.request_id and status in ('intake-started', 'intake-accpted') order by id asc limit 1) as pages
