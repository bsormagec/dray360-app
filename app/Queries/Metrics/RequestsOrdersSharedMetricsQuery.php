...
class RequestsOrdersSharedMetricsQuery
{
    ...
    protected function getBaseData(): Collection
    {
        ...
        return DB::table('t_orders ', 'o')
            ...
            ->addSelect(DB::raw("(select json_extract(s.status_metadata, '$.original_filename') from t_job_state_changes as s where s.status in ('intake-accepted', 'intake-accepted-datafile') and o.request_id = s.request_id order by id asc limit 1) as original_filename"))
            ->addSelect(DB::raw("exists(select id from t_job_state_changes as s where s.status in ('intake-accepted', 'intake-accepted-datafile') and o.request_id = s.request_id and lower(json_unquote(json_extract(s.status_metadata, '$.original_filename')))     like '%pdf' order by id asc limit 1) as is_pdf"))
            ->addSelect(DB::raw("exists(select id from t_job_state_changes as s where s.status in ('intake-accepted', 'intake-accepted-datafile') and o.request_id = s.request_id and lower(json_unquote(json_extract(s.status_metadata, '$.original_filename'))) not like '%pdf' order by id asc limit 1) as is_datafile"))
